...
def main(choice = 'RR'):
    ...
    with open('{0}/N{1}_kavg{2}_rep{3}_{4}_crucial_morep.dat'.format(wdir, N, k_avg, rep, choice), 'w') as f:
        f.write('# created on %s\n' %time.strftime("%H:%M\t%d/%m/%Y"))
        f.write('# p*<k>\t frac_lmcc\n')
        count = 0
        for p in np.linspace(0.1, 0.9, num=100):# different p same graph
            count += 1
            print(count)
            fraction_list = []
            ...
            for i in range(rep):
                ...
                G.remove_corresp(G.attack_crucial(Q=to_be_removed))
                G.cascade()
