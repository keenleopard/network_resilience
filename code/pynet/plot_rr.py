...
#plt.subplot(2,1,1)
data = np.genfromtxt('./analysis/avg_sd_RR_1000.dat',delimiter='\t', skip_header=True)
d1 = np.asarray(group_into_four(data))
plt.plot(d1[:,0], d1[:,1], '^-', color='#191970', label='N = 1000')

data = np.genfromtxt('./analysis/avg_sd_RR_2000.dat',delimiter='\t', skip_header=True)
d2 = np.asarray(group_into_four(data))
plt.plot(d2[:,0], d2[:,1], color='#4169e1', label='N = 2000')

data = np.genfromtxt('./analysis/avg_sd_RR_4000.dat',delimiter='\t', skip_header=True)
d4 = np.asarray(group_into_four(data))
plt.plot(d4[:,0], d4[:,1], color='#00ced1', label='N = 4000')

data = np.genfromtxt('./analysis/avg_sd_RR_8000.dat',delimiter='\t', skip_header=True)
d8 = np.asarray(group_into_four(data))
plt.plot(d8[:,0], d8[:,1], color='#228b22', label='N = 8000')

data = np.genfromtxt('./analysis/avg_sd_RR_16000.dat',delimiter='\t', skip_header=True)
d16 = np.asarray(group_into_four(data))
plt.plot(d16[:,0], d16[:,1], color='#daa520', label='N = 16000')

data = np.genfromtxt('./analysis/avg_sd_RR_32000.dat',delimiter='\t', skip_header=True)
...
plt.ylabel(r'$P_\infty$')
'''
plt.savefig('../../plot/RR_cluster_generated_data_into4.pdf', bbox_inches='tight')
