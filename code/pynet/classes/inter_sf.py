...
def scale_free_powerlaw(n = 4000, lam = 3, k_avg = 4) :
    ...
    while(1): 
        ...
        if (nx.is_valid_degree_sequence(s)) :
            while(1):
	        G = nx.configuration_model(s) # self-loops
                n_selfloop_edges = len(G.selfloop_edges())
                n_selfloop_nodes = len(G.nodes_with_selfloops())
                if (n_selfloop_nodes/n < 0.01) and (n_selfloop_edges - n_selfloop_nodes < n_selfloop_nodes) : break
                #elif n_selfloop_nodes/n >= 0.01:
                    #print("too many selfloop nodes")
                #else: print("too many selfloop edges on one node")
            break
